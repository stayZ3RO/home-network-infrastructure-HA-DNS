...
- Configured keepalived for Virtual IP failover
...
- Deployed Prometheus on the monitoring host
- Deployed Grafana for dashboards and data exploration
- Deployed Alertmanager for alert handling and notification routing
...
- Configured Alertmanager to route alerts to Discord
- Added Discord webhook delivery for the `homelab-alerts` server/channel
- Stored the Discord webhook URL in a local secret file
- Mounted the webhook secret into the Alertmanager container as read-only
- Added `.gitignore` protection for Alertmanager secrets
- Fixed Alertmanager secret visibility by recreating the container after adding the new volume mount
...
- Expand into Proxmox-based infrastructure services
